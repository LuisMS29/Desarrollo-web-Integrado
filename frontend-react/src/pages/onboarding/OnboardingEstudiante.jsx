...
export default function OnboardingEstudiante() {
  const { user, refreshProfile } = useAuth();
  const { notifySuccess, notifyError } = useToast();
  const navigate = useNavigate();

  const [form, setForm] = useState({
    nombres: "", apellidos: "", dni: "", fechaNacimiento: "", direccion: "", telefono: "",
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await estudiantePanelApi.actualizarMiPerfil(form);
      ...
      <div style={{ position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", height: "100dvh", overflow: "hidden" }}>
        ...
        >
          <div style={{ ...fadeUp(phase >= 1), textAlign: "center", maxWidth: 480 }}>
            ...
            <p style={{ color: "#cbd5e1", fontSize: "clamp(0.8rem, 1vw, 1.05rem)", lineHeight: 1.85 }}>
              Antes de entrar al panel, necesitamos algunos datos tuyos para completar tu registro como estudiante.
            </p>
          </div>
        </div>
        ...
        >
          <div style={{
            ...
            <h2 className="font-display fw-bold" style={{ fontSize: "1.2rem", marginBottom: 4 }}>Datos del estudiante</h2>
            <p className="text-secondary mb-4" style={{ fontSize: "0.85rem" }}>Todos los campos marcados con * son obligatorios.</p>

            <form onSubmit={handleSubmit} noValidate>
              {[ 
                { key: "nombres", label: "Nombres *", placeholder: "Ej. Luis Fernando" },
                { key: "apellidos", label: "Apellidos *", placeholder: "Ej. Castillo Farro" },
              ...
              <div style={{ ...fadeUp(phase >= 2, "0.18s"), marginBottom: 14 }}>
                <label className="form-label">Fecha de nacimiento</label>
                <input type="date" className="form-control" value={form.fechaNacimiento} onChange={handleChange("fechaNacimiento")} disabled={saving} />
              </div>

              {[
                { key: "direccion", label: "Dirección", placeholder: "Ej. Av. Los Olivos 123" },
                { key: "telefono", label: "Teléfono", placeholder: "Ej. 987654321" },
              ].map((f, i) => (
                <div key={f.key} style={{ ...fadeUp(phase >= 3, `${i * 0.06}s`), marginBottom: 14 }}>
                  <label className="form-label">{f.label}</label>
                  <input type="text" className="form-control" value={form[f.key]} onChange={handleChange(f.key)} disabled={saving} placeholder={f.placeholder} />
                </div>
              ))}

              <div style={{ ...fadeUp(phase >= 3, "0.12s"), marginTop: 20 }}>
